...
var NDKRelaySubscription = class {
  ...
  onevent(event) {
    ...
    for (const { subscription } of this.items.values()) {
      if ((0, import_nostr_tools.matchFilters)(subscription.filters, event)) {
...
var NDKRelay = class _NDKRelay extends import_tseep.EventEmitter {
  url;
  scores;
  connectivity;
  subs;
...
function encode() {
  ...
  if (this.isParamReplaceable()) {
    return import_nostr_tools3.nip19.naddrEncode({
      kind: this.kind,
      pubkey: this.pubkey,
    ...
    return import_nostr_tools3.nip19.neventEncode({
      id: this.tagId(),
      relays,
      author: this.pubkey
    });
  } else {
    return import_nostr_tools3.nip19.noteEncode(this.tagId());
  }
}
...
function verifySignature(persist) {
  ...
  try {
    if (this.ndk?.asyncSigVerification) {
      ...
      const hash = (0, import_sha256.sha256)(new TextEncoder().encode(this.serialize()));
      const res = import_secp256k1.schnorr.verify(this.sig, hash, this.pubkey);
      if (res)
        verifiedSignatures.set(this.id, this.sig);
      else
        verifiedSignatures.set(this.id, false);
...
function getEventHashFromSerializedEvent(serializedEvent) {
  const eventHash = (0, import_sha256.sha256)(new TextEncoder().encode(serializedEvent));
...
var NDKEvent = class _NDKEvent extends import_tseep2.EventEmitter {
  ndk;
  created_at;
  content = "";
  tags = [];
  ...
  async publish(relaySet, timeoutMs, requiredRelayCount) {
    ...
    this.ndk.subManager.subscriptions.forEach((sub) => {
      if (sub.filters.some((filter) => (0, import_nostr_tools4.matchFilter)(filter, rawEvent))) {
...
function filterForEventsTaggingId(id) {
  try {
    const decoded = import_nostr_tools5.nip19.decode(id);
    switch (decoded.type) {
      case "naddr":
        return {
          "#a": [
...
function filterFromId(id) {
  ...
  if (id.match(BECH32_REGEX)) {
    try {
      decoded = import_nostr_tools5.nip19.decode(id);
      switch (decoded.type) {
        case "nevent": {
...
function relaysFromBech32(bech322, ndk) {
  try {
    const decoded = import_nostr_tools5.nip19.decode(bech322);
...
var NDKUser = class _NDKUser {
  ...
  get npub() {
    if (!this._npub) {
      ...
      this._npub = import_nostr_tools6.nip19.npubEncode(this.pubkey);
    }
    return this._npub;
  }
  set npub(npub) {
  ...
  get pubkey() {
    if (!this._pubkey) {
      ...
      this._pubkey = import_nostr_tools6.nip19.decode(this.npub).data;
    }
    return this._pubkey;
  }
  /**
  ...
  async getArticle() {
    ...
    switch (articleTag[0]) {
      case "a":
        ...
        taggedBech32 = import_nostr_tools7.nip19.naddrEncode({ kind: parseInt(kind), pubkey, identifier });
        break;
      case "e":
        taggedBech32 = import_nostr_tools7.nip19.noteEncode(articleTag[1]);
        break;
      case "r":
...
var NDKPrivateKeySigner = class _NDKPrivateKeySigner {
    ...
    if (privateKey) {
      if (typeof privateKey === "string") {
        if (privateKey.startsWith("nsec1")) {
          const { type, data } = import_nostr_tools9.nip19.decode(privateKey);
          if (type === "nsec")
            this._privateKey = data;
        } else if (privateKey.length === 64) {
          this._privateKey = (0, import_utils3.hexToBytes)(privateKey);
        } else {
      ...
      if (this._privateKey) {
        this._user = new NDKUser({
          pubkey: (0, import_nostr_tools8.getPublicKey)(this._privateKey)
        });
      }
    }
  }
  get privateKey() {
    if (!this._privateKey)
      return void 0;
    return (0, import_utils3.bytesToHex)(this._privateKey);
  }
  static generate() {
    const privateKey = (0, import_nostr_tools8.generateSecretKey)();
  ...
  async sign(event) {
    ...
    return (0, import_nostr_tools8.finalizeEvent)(event, this._privateKey).sig;
  }
  async encrypt(recipient, value) {
    ...
    return await import_nostr_tools8.nip04.encrypt(this._privateKey, recipientHexPubKey, value);
  }
  async decrypt(sender, value) {
    ...
    return await import_nostr_tools8.nip04.decrypt(this._privateKey, senderHexPubKey, value);
  }
};
...
var NDKNwc = class _NDKNwc extends import_tseep4.EventEmitter {
  ...
  constructor({
    ...
    this.signer = new NDKPrivateKeySigner(
      secret instanceof Uint8Array ? secret : (0, import_utils4.hexToBytes)(secret)
    );
...
function disconnect(pool, debug8) {
  debug8 ??= (0, import_debug7.default)("ndk:relay:auth-policies:disconnect");
  return async (relay) => {
...
function signIn({ ndk, signer, debug: debug8 } = {}) {
  debug8 ??= (0, import_debug7.default)("ndk:auth-policies:signIn");
...
var NDKNip46Backend = class {
  ...
  constructor(ndk, privateKeyOrSigner, permitCallback, relayUrls) {
    ...
    if (privateKeyOrSigner instanceof Uint8Array) {
      ...
      this.signer = new NDKPrivateKeySigner((0, import_utils5.hexToBytes)(privateKeyOrSigner));
...
var NDKNip46Signer = class extends import_tseep6.EventEmitter {
  ndk;
  remoteUser;
  remotePubkey;
  token;
...
var import_typescript_lru_cache2 = require("typescript-lru-cache");
...
var OutboxTracker = class extends import_tseep7.EventEmitter {
  data;
  ndk;
  debug;
  constructor(ndk) {
...
var NDKPool = class extends import_tseep8.EventEmitter {
...
var debug6 = (0, import_debug9.default)("ndk:active-user");
...
async function generateZapRequest(target, ndk, data, pubkey, amount, relays, comment, tags, signer) {
  ...
  const zapRequest = import_nostr_tools10.nip57.makeZapRequest({
    profile: pubkey,
...
// src/zapper/index.ts
var d3 = (0, import_debug10.default)("ndk:zapper");
...
var NDK = class extends import_tseep10.EventEmitter {
  explicitRelayUrls;
  pool;
  outboxPool;
  _signer;
  ...
  constructor(opts = {}) {
    super();
    this.debug = opts.debug || (0, import_debug11.default)("ndk");
