...
tests.push(async () => {
    ...
    if (chan_id.get_a().length != 32)
        return false;
    if (chan_id.get_a()[31] != 4)
        return false;
    return true;
});
tests.push(async () => {
...
var seed_counter = 0;
class Node {
    constructor(chan_man, tx_broadcasted, logger, keys_manager, net_graph) {
...
function get_chanman() {
    const fee_est = ldk.FeeEstimator.new_impl({
        get_est_sat_per_1000_weight(_confirmation_target) {
            return 253;
        }
    });
    var tx_broadcaster;
    const tx_broadcasted = new Promise((resolve, _reject) => {
        tx_broadcaster = ldk.BroadcasterInterface.new_impl({
            broadcast_transactions(txn) {
                for (const tx of txn) {
    ...
    const persister = ldk.Persist.new_impl({
        persist_new_channel(_channel_id, _data) {
            return ldk.ChannelMonitorUpdateStatus.LDKChannelMonitorUpdateStatus_Completed;
        },
        update_persisted_channel(_channel_id, _update, _data) {
            return ldk.ChannelMonitorUpdateStatus.LDKChannelMonitorUpdateStatus_Completed;
        }
    });
    ...
    const chan_man = ldk.ChannelManager.constructor_new(fee_est, chain_watch, tx_broadcaster, router.as_Router(), logger, keys_manager.as_EntropySource(), keys_manager.as_NodeSigner(), keys_manager.as_SignerProvider(), config, params, 42);
...
tests.push(async () => {
    ...
    if (chan_create_err.is_ok())
        return false;
    if (!(chan_create_err instanceof ldk.Result_ChannelIdAPIErrorZ_Err))
        return false;
...
tests.push(async () => {
    ...
    const pm_a = ldk.PeerManager.constructor_new(a.chan_man.as_ChannelMessageHandler(), ignorer.as_RoutingMessageHandler(), ignorer.as_OnionMessageHandler(), ignorer.as_CustomMessageHandler(), 0xdeadbeef, a.keys_manager.as_EntropySource().get_secure_random_bytes(), a.logger, a.keys_manager.as_NodeSigner());
    const pm_b = ldk.PeerManager.constructor_new(b.chan_man.as_ChannelMessageHandler(), ignorer.as_RoutingMessageHandler(), ignorer.as_OnionMessageHandler(), ignorer.as_CustomMessageHandler(), 0xdeadbeef, b.keys_manager.as_EntropySource().get_secure_random_bytes(), b.logger, b.keys_manager.as_NodeSigner());
    var sock_b;
...
tests.push(async () => {
    ...
    const om_handler_a = ldk.CustomOnionMessageHandler.new_impl({
        read_custom_message(message_type, buffer) {
            ...
            return ldk.Result_COption_OnionMessageContentsZDecodeErrorZ.constructor_ok(ldk.Option_OnionMessageContentsZ.constructor_some(ldk.OnionMessageContents.new_impl({
                tlv_type() { return 9998n; },
                write() { throw new Error(); },
                debug_str() { return "Message Contents"; }
            })));
        },
    ...
    const underlying_om_a = ldk.OnionMessenger.constructor_new(a.keys_manager.as_EntropySource(), a.keys_manager.as_NodeSigner(), a.logger, a.chan_man.as_NodeIdLookUp(), a_msg_router, ignorer.as_OffersMessageHandler(), ignorer.as_AsyncPaymentsMessageHandler(), om_handler_a);
    ...
    const om_b = ldk.OnionMessenger.constructor_new(b.keys_manager.as_EntropySource(), b.keys_manager.as_NodeSigner(), b.logger, b.chan_man.as_NodeIdLookUp(), msg_router_b, ignorer.as_OffersMessageHandler(), ignorer.as_AsyncPaymentsMessageHandler(), om_handler_b);
    const pm_a = ldk.PeerManager.constructor_new(a.chan_man.as_ChannelMessageHandler(), ignorer.as_RoutingMessageHandler(), om_a, ignorer.as_CustomMessageHandler(), 0xdeadbeef, a.keys_manager.as_EntropySource().get_secure_random_bytes(), a.logger, a.keys_manager.as_NodeSigner());
    const pm_b = ldk.PeerManager.constructor_new(b.chan_man.as_ChannelMessageHandler(), ignorer.as_RoutingMessageHandler(), om_b.as_OnionMessageHandler(), ignorer.as_CustomMessageHandler(), 0xdeadbeef, b.keys_manager.as_EntropySource().get_secure_random_bytes(), b.logger, b.keys_manager.as_NodeSigner());
    var sock_b;
    const sock_a = ldk.SocketDescriptor.new_impl({
        send_data(data, resume_read) {
            assert(pm_b.read_event(sock_b, data) instanceof ldk.Result_boolPeerHandleErrorZ_OK);
    ...
    assert(pm_a.list_peers().length == 1);
    assert(pm_b.list_peers().length == 1);
    underlying_om_a.send_onion_message(ldk.OnionMessageContents.new_impl({
        tlv_type() { return 4242n; },
        write() {
        ...
        debug_str() { return "Onion Message A Contents"; }
    ...
    om_b.send_onion_message(ldk.OnionMessageContents.new_impl({
        tlv_type() { return 4343n; },
        write() {
